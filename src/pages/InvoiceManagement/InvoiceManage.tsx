...
const InvoiceManager = () => {
  ...
  return (
    <div className="invoice-container">
      ...
      {/* =======================================================
          [MỚI] MODAL 1: NHẬP SỐ ĐIỆN NƯỚC (ĐÃ SỬ DỤNG APPMODAL)
          ======================================================= */}
      <AppModal
        open={showReadingModal && Boolean(selectedInvoice)}
        onClose={() => setShowReadingModal(false)}
        title={`Sửa chỉ số - ${selectedInvoice ? getRoomName(selectedInvoice) : ''}`}
        icon={<Edit3 size={18} />}
        size="md"
        ...
        footer={
          <>
            ...
            <button type="submit" form="reading-form" className="btn btn-primary">Lưu & Tính tổng tiền</button>
          </>
        }
      >
        <form id="reading-form" onSubmit={handleSaveReading} style={{ display: 'flex', flexDirection: 'column', margin: 0 }}>
          {/* KHU VỰC ĐIỆN */}
          {elecServiceInfo && (
      ...
      {/* =======================================================
          [MỚI] MODAL 2: CHI TIẾT HÓA ĐƠN (ĐÃ SỬ DỤNG APPMODAL)
          ======================================================= */}
      <AppModal
        open={showDetailModal && Boolean(selectedInvoice)}
        onClose={() => setShowDetailModal(false)}
        title={`Chi tiết ${selectedInvoice?.type === 'Periodic' ? 'Hóa đơn Định kỳ' : 'Hóa đơn Phát sinh'}`}
        icon={<FileText size={18} />}
        size="lg"
        headerClassName="mr-app-modal-header mr-app-modal-header--detail"
        footer={
          <button type="button" className="btn btn-outline" onClick={() => setShowDetailModal(false)}>Đóng lại</button>
        }
      >
        {selectedInvoice && (
      ...
      {/* =======================================================
          ...
          ======================================================= */}
      <AppModal
        open={showBulkReadingModal}
        onClose={() => setShowBulkReadingModal(false)}
        title="Ghi Chỉ Số Điện Nước Hàng Loạt"
        icon={<Droplet size={18} />}
        size="xl"
        headerClassName="mr-app-modal-header mr-app-modal-header--detail"
        footer={
          <>
            <button type="button" className="btn btn-outline" onClick={() => setShowBulkReadingModal(false)}>Đóng tạm</button>
            <button type="button" className="btn btn-primary" onClick={handleSaveBulkReadings} disabled={loading}>
