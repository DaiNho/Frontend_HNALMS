...
import CreateManagerNotification from './CreateManagerNotification';
import DisableNotification from './DisableNotification';
...
export default function NotificationList() {
    ...
    const fetchCounts = async () => {
      try {
        const [allRes, draftRes, sentRes] = await Promise.all([
          ...
          notificationService.getMyNotifications({ page: 1, limit: 1, status: 'draft' }),
          notificationService.getMyNotifications({ page: 1, limit: 1, status: 'sent' }),
        ]);
        setAllCount(allRes.data?.pagination?.total_count ?? 0);
        setDraftCount(draftRes.data?.pagination?.total_count ?? 0);
        setSentCount(sentRes.data?.pagination?.total_count ?? 0);
      } catch {
        // silent
      }
    };
  ...
  const handleViewPublish = async (id: string) => {
    try {
  ...
  return (
    <div className="notification-container">
            ...
            <div className="notification-title-row">
              ...
              <div className="notification-title-text">
                ...
                <p className="notification-subtitle">
                  Tạo và gửi thông báo cho nhân sự trong Tòa Nhà Hoàng Nam.
                </p>
              </div>
            </div>
          </div>
      ...
      <AppModal
        ...
        size="lg"
        footer={
          viewNotification?.status === 'draft' ? (
            <>
              <button type="button" className="ms-btn ms-btn--ghost" onClick={() => setViewNotification(null)}>
                Đóng
              </button>
              <button
                type="button"
                className="ms-btn ms-btn--primary"
                onClick={() => handleViewPublish(viewNotification._id)}
              >
                <Send size={16} />
                Phát hành
              </button>
        ...
        {viewNotification && (
          <div>
            <div className="notification-detail-meta">
              <span className={`status-badge ${viewNotification.status === 'draft' ? 'status-draft-badge' : viewNotification.status === 'sent' ? 'status-sent-badge' : 'status-archived-badge'}`}>
                {viewNotification.status === 'draft' ? 'Bản nháp' :
                  viewNotification.status === 'sent' ? 'Đã gửi' : 'Đã lưu trữ'}
              </span>
              <span className="notification-detail-date">
                <Clock size={13} /> {formatDate(viewNotification.createdAt)}
              </span>
            </div>
            <h3 className="notification-detail-title">{viewNotification.title}</h3>
            <div className="notification-detail-content">{viewNotification.content}</div>
          </div>
        )}
      </AppModal>
        ...
        {publishNotification && (
          <div className="ms-delete-notice" style={{ background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
            ...
            <p className="ms-delete-notice-text" style={{ color: '#14532d' }}>
              Bạn có chắc chắn muốn phát hành thông báo "<strong>{publishNotification.title}</strong>"? Sau khi phát hành sẽ không thể chỉnh sửa hay xóa.
            </p>
          </div>
        )}
      </AppModal>
